...
{
  D->FirPtrs.Pnt    = MemoryAllocate(D->FrameHdr.NrOfChannels, sizeof(*D->FirPtrs.Pnt));
  D->FirPtrs.Status = AllocateArray(2, sizeof(**D->FirPtrs.Status), 
     D->FrameHdr.NrOfChannels, (1 << SIZE_CODEDPREDORDER));
  
  D->FrameHdr.HalfProb = 
    MemoryAllocate(D->FrameHdr.NrOfChannels, sizeof(*D->FrameHdr.HalfProb));
  D->FrameHdr.NrOfHalfBits = 
    MemoryAllocate(D->FrameHdr.NrOfChannels, sizeof(*D->FrameHdr.NrOfHalfBits));  
  D->FrameHdr.PredOrder = 
    MemoryAllocate(D->FrameHdr.MaxNrOfFilters, sizeof(*D->FrameHdr.PredOrder));  
  D->FrameHdr.PtableLen = 
    MemoryAllocate(D->FrameHdr.MaxNrOfPtables, sizeof(*D->FrameHdr.PtableLen));  
  D->FrameHdr.ICoefA = 
    AllocateArray (
  ...
  D->PredicVal = AllocateArray (2, sizeof(**D->PredicVal), D->FrameHdr.NrOfChannels, D->FrameHdr.NrOfBitsPerCh );
