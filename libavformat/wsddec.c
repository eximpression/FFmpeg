...
static int wsd_probe(AVProbeData *p)
{
    if (p->buf_size < 45 || memcmp(p->buf, "1bit", 4) ||
        (p->buf[8] >= 0x10 && (AV_RB32(p->buf + 20) < 0x80 || AV_RB32(p->buf + 24) < 0x80)) ||
...
static int wsd_to_av_channel_layout(AVFormatContext *s, int bit)
{
    switch (bit) {
...
static int wsd_read_header(AVFormatContext *s)
{
    ...
    AVStream *st;
    int version;
    uint32_t text_offset, data_offset, playback_time, channel_assign;
    ...
    if (!(channel_assign & 1)) {
        ...
        for (i = 1; i < 32; i++)
            if (channel_assign & (1 << i))
                st->codec->channel_layout |= wsd_to_av_channel_layout(s, i);
    } else {
        av_log(s, AV_LOG_INFO, "arbitary channel layout\n");
    }
