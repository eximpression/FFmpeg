...
scarletbook_handle_t *scarletbook_open(AVFormatContext *s)
{
    ...
    //no need to read frame.data here
    /*
#ifdef __lv2ppu__
    ...
    if (sb->master_toc->area_1_toc_1_start > 0) // Area 1 (TWOCHTOC) TOC-1
    {
        ...
        if (sb->area[sb->area_count].area_data == NULL)
        {
            ...
            av_log(s, AV_LOG_ERROR, "Error: Can't alocate memory for Area 1 (TWOCHTOC) TOC-1 !!");
        }
        else
        {
            if (!sacd_read_block_raw(pb, sb->master_toc->area_1_toc_1_start,(uint32_t) sb->master_toc->area_1_toc_size, sb->area[sb->area_count].area_data) )
            {
                fwprintf(stderr, L"Can't read Area 1 (TWOCHTOC) TOC-1 !! Trying to read and use TOC-2...\n");
                av_log(s, AV_LOG_WARNING, "Warning: Can't read Area 1 (TWOCHTOC) TOC-1 !! Trying to read and use TOC-2...");
                flag_use_toc2 = 1;
            }
            else
              flag_use_toc1 = 1;
            ...
            if (sb->master_toc->area_1_toc_2_start > 0) // Area 1 (TWOCHTOC) TOC-2
            {
                ...
                if (sb->area[2].area_data == NULL)
                {
                    ...
                    av_log(s, AV_LOG_ERROR, "Error: Can't alocate memory for backup Area 1 (TWOCHTOC) TOC-2.");
                    flag_use_toc2 = 0;
                }
                else
                {
                    if (!sacd_read_block_raw(pb, sb->master_toc->area_1_toc_2_start, (uint32_t)sb->master_toc->area_1_toc_size, sb->area[2].area_data))
                    {
                        fwprintf(stderr, L"Warning: can't read Area 1 (TWOCHTOC) TOC-2 !! There are some errros on disc !\n");
                        av_log(s, AV_LOG_WARNING, "Warning: can't read Area 1 (TWOCHTOC) TOC-2 !! There are some errros on disc !");
                        flag_use_toc2 = 0;
                    }
                    else  // compare
                    {
                        ...
                        if (res_cmp != 0x00)
                        {
                            ...
                            av_log(s, AV_LOG_WARNING, "Warning: Area 1 (TWOCHTOC) TOC-1 did not match with Area 1 (TWOCHTOC) TOC-2. Disc has some errors !! Using TOC-1... ");
                            flag_use_toc1 = 1;
                            flag_use_toc2 = 0;
                        }
                    }
    ...
    if (sb->master_toc->area_2_toc_1_start > 0) //  Area 2 (MULCHTOC) TOC-1
    {
        ...
        if (!sb->area[sb->area_count].area_data)
        {
            ...
            av_log(s, AV_LOG_ERROR, "Error: can't alocate memory for Area 2 (MULCHTOC) TOC-1 !!");
        }
        else
        {
            if (!sacd_read_block_raw(pb, sb->master_toc->area_2_toc_1_start, (uint32_t)sb->master_toc->area_2_toc_size, sb->area[sb->area_count].area_data))
            {
                fwprintf(stderr, L"Error: can't read Area 2 (MULCHTOC) TOC-1 !! Trying to read and use TOC-2...\n");
                av_log(s, AV_LOG_ERROR, "Error: can't read Area 2 (MULCHTOC) TOC-1 !! Trying to read and use TOC-2...");
                flag_use_toc2 = 1;
            }
            else
                flag_use_toc1 = 1;
            ...
            if (sb->master_toc->area_2_toc_2_start > 0) // Area 2 (MULCHTOC) TOC-2
            {
                ...
                {
                    if (!sacd_read_block_raw(pb, sb->master_toc->area_2_toc_2_start, (uint32_t)sb->master_toc->area_2_toc_size, sb->area[3].area_data))
                    {
                        fwprintf(stderr, L"Warning: can't read Area 2 (MULCHTOC) TOC-2 !! There are some errros on disc !\n");
                        av_log(s, AV_LOG_WARNING, "Warning: can't read Area 2 (MULCHTOC) TOC-2 !! There are some errros on disc !");
                        flag_use_toc2 = 0;
                    }
                    else // compare
                    {
...
static int scarletbook_read_master_toc(scarletbook_handle_t *handle)
{
    int          i;
    uint8_t      * p;
    AVFormatContext *s = (AVFormatContext *)handle->format_ctx;
    AVIOContext *pb = s->pb;
    master_toc_t *master_toc;

    int64_t mtoc_pos = avio_seek(pb,START_OF_MASTER_TOC * SACD_LSN_SIZE, SEEK_SET);
    handle->master_data = malloc(MASTER_TOC_LEN * SACD_LSN_SIZE);
    if (!handle->master_data)
        return AVERROR_BUFFER_TOO_SMALL;

    if (avio_read(pb, handle->master_data, MASTER_TOC_LEN * SACD_LSN_SIZE) != MASTER_TOC_LEN * SACD_LSN_SIZE) {
    ...
    {
        fwprintf(stderr, L"scarletbook_read_master_toc: Not a ScarletBook disc!\n");
        av_log(s, AV_LOG_ERROR, "scarletbook_read_master_toc: Not a ScarletBook disc!");
        return AVERROR_INVALIDDATA;
    }
                        ...
                        for (j = 0; j < track_amount; j++)
                        {
                            ...
                            if (*track_ptr != 0)
                            {
                                ...
                                if (track_text_len > 255)
                                {
                                    ...
                                    av_log(s, AV_LOG_ERROR, "Error: The lenght of track text is bigger than 255!!; area_idx=%d; track_type=0x%02x; track number=%d", area_idx, track_type, i + 1);
                                    break;
                                }
                                ...
                                for(int te=0; te < (int)strlen(track_ptr); te++)
                                {
                                    if(*(uint8_t*)track_ptr1 < (uint8_t)0x20)
                                    {
                                        *((uint8_t *)track_ptr1) = (uint8_t)0x20;
                                        ...
                                        av_log(s, AV_LOG_WARNING, "Warning: Illegal char in the track text! Corrected. ;area_idx=%d; track_type=0x%02x; track number=%d", area_idx, track_type, i + 1);
                                    }
                                    track_ptr1 ++;                                         
                                }
                                //DEBUG
                                ...
                                if(track_text_converted_ptr == NULL || strlen(track_text_converted_ptr)==0)
                                {
                                    ...
                                    av_log(s, AV_LOG_ERROR, "Error: Cannot convert to UTF8 the track text!!; track_type=0x%02x; track number=%d", track_type, i+1);
                                }
                                //DEBUG
                                ...
                                default:
                                    ...
                                    av_log(s, AV_LOG_ERROR, "Error : scarletbook_read_area_toc(), Unknown track text type: 0x%02x", track_type);
                                    break;
                                }
                            }
                            if (j < track_amount - 1)
...
int scarletbook_process_frames(scarletbook_handle_t *handle, uint8_t *read_buffer, int blocks_read_in, int last_block, frame_read_callback_t frame_read_callback, void *userdata)
{
    ...
    {            
        ...
        if(handle->audio_sector.header.packet_info_count > (uint8_t)7)  // max 7 packets must contain an audio sector
        {
            ...
            av_log(s, AV_LOG_ERROR, "Error : scarletbook_process_frames(). > Max 7 packets!!, handle->audio_sector.header.packet_info_count:%d", handle->audio_sector.header.packet_info_count);
        }
                ...
                case DATA_TYPE_AUDIO:
                    if (packet->frame_start)
                    {
                        ...
                        if (frametimecode_prev > 0)
                        {
                            ...
                            {
                                av_log(s, AV_LOG_ERROR, "Error : scarletbook_process_frames(), frametimecode not succesive! frametimecode_current:%u, frametimecode_prev:%u", frametimecode_current, frametimecode_prev);
                            }
                        }                       
                    ...
                    if (handle->frame.started)
                    {
                        ...
                        else
                        {
                            ...
                            av_log(s, AV_LOG_ERROR, "Error : scarletbook_process_frames(), buffer overflow error. in blocks_read:%d", j);
                        }
                    }
                    break;
...
int sacd_read_block_raw(AVIOContext *pb, uint32_t lb_number,
                             uint32_t block_count, uint8_t *data)
{
    int64_t position = avio_seek(pb, lb_number * SACD_LSN_SIZE, SEEK_SET);
    if (position != lb_number * SACD_LSN_SIZE) {
        return 0;
    }
    int read_size = avio_read(pb,data,block_count * SACD_LSN_SIZE);
