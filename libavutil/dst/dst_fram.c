...
#define HALF_VALUE 0b100000000000
...
{
    int FilterNr, FilterLength, TableNr, k, i, j,cvalue,TableNr8;
    ...
    for (FilterNr = 0; FilterNr < D->FrameHdr.NrOfFilters; FilterNr++)
    {
        ...
        for (TableNr = 0; TableNr < 16; TableNr++)
        {
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                for (j = 0; j < k; j++)
                {
                    cvalue += (((i >> j) & 1) * 2 - 1) * D->FrameHdr.ICoefA[FilterNr][TableNr8 + j];
                }
...
int DST_FramDSTDecode(uint8_t *DSTdata, uint8_t *MuxedDSDdata, int FrameSizeInBytes, int FrameCnt, ebunch *D)
{
    ...
    if (D->FrameHdr.DSTCoded == 1)
    {
        ...
        //uint16_t LT_ICoefU[2 * MAX_CHANNELS][16][256] __attribute__((aligned(16)));
        ...
        {
            ByteNr = BitNr >> 3; // == BitNr / 8;
            BitNrr = 7 - BitNr&7; // == BitNr % 8;
            if (NrOfChannels == 2) {
                
                ////////////////1
                Filter = D->FrameHdr.Filter4Bit[0][BitNr];

                /* Calculate output value of the FIR filter */
#define F(x) LT_ICoefI[Filter][(x)][LT_Status[0][(x)]]
                Predict = F( 0) + F( 1) + F( 2) + F( 3) +
                                    F( 4) + F( 5) + F( 6) + F( 7) +
                                    F( 8) + F( 9) + F(10) + F(11) +
                                    F(12) + F(13) + F(14) + F(15);
#undef F
                /* Arithmetic decode the incoming bit */
                if ((D->FrameHdr.HalfProb[0]/* == 1*/) && (BitNr < D->FrameHdr.NrOfHalfBits[0]))
                {
                    LT_ACDecodeBit_Decode(&AC, &Residual, HPROBS , D->AData, D->ADataLen);
                }
                else
                {
                    const int table4bit = D->FrameHdr.Ptable4Bit[0][BitNr];
                ...
                MuxedDSD[ByteNr * NrOfChannels + 0] |= (uint8_t)(BitVal << BitNrr);

                /* Update filter */
                {
                    uint32_t* const st = (uint32_t*)LT_Status[0];
                ...
                ///////////////////////////2
                Filter = D->FrameHdr.Filter4Bit[1][BitNr];

                /* Calculate output value of the FIR filter */
#define F(x) LT_ICoefI[Filter][(x)][LT_Status[1][(x)]]
                Predict = F( 0) + F( 1) + F( 2) + F( 3) +
                                    F( 4) + F( 5) + F( 6) + F( 7) +
                                    F( 8) + F( 9) + F(10) + F(11) +
                                    F(12) + F(13) + F(14) + F(15);
#undef F
                ...
                /* Arithmetic decode the incoming bit */
                if ((D->FrameHdr.HalfProb[1]/* == 1*/) && (BitNr < D->FrameHdr.NrOfHalfBits[1]))
                {
                    LT_ACDecodeBit_Decode(&AC, &Residual, HPROBS , D->AData, D->ADataLen);
                }
                else
                {
                    const int table4bit = D->FrameHdr.Ptable4Bit[1][BitNr];
                    const int PtableIndex = LT_ACGetPtableIndex(Predict, D->FrameHdr.PtableLen[table4bit]);
                ...
                /* Shift the result into the correct bit position */ \
                MuxedDSD[ByteNr * NrOfChannels + 1] |= (uint8_t)(BitVal << BitNrr);

                /* Update filter */
                {
                    uint32_t* const st = (uint32_t*)LT_Status[1];
                    st[3] = (st[3] << 1) | ((st[2] >> 31) & 1);
                    st[2] = (st[2] << 1) | ((st[1] >> 31) & 1);
                    st[1] = (st[1] << 1) | ((st[0] >> 31) & 1);
                    st[0] = (st[0] << 1) | BitVal;
                ...
                for (ChNr = 0; ChNr < NrOfChannels; ChNr++)
                {
                    Filter = D->FrameHdr.Filter4Bit[ChNr][BitNr];

                    /* Calculate output value of the FIR filter */
                   // LT_RUN_FILTER_I(LT_ICoefI[Filter], LT_Status[ChNr]);
    #define F(x) LT_ICoefI[Filter][(x)][LT_Status[ChNr][(x)]]
                    Predict = F( 0) + F( 1) + F( 2) + F( 3) +
                                        F( 4) + F( 5) + F( 6) + F( 7) +
                                        F( 8) + F( 9) + F(10) + F(11) +
                                        F(12) + F(13) + F(14) + F(15);
    #undef F
                    ...
                    /* Shift the result into the correct bit position */ \
                    MuxedDSD[ByteNr * NrOfChannels + ChNr] |= (uint8_t)(BitVal << BitNrr);
